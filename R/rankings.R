...
add_cron_similarity <- function(
  df,
  cols,
  target_name = CRON_PLAYER,
  out_col = "CRON"
) {
...
CATEGORY_DEFS <- list(
  hitting = list(
    AB = list(
      actual = function(df) df$AB,
      proj = function(df) df$proj_AB,
      flip = 1
    ),
    H = list(
      actual = function(df) df$H,
      proj = function(df) df$proj_H,
      flip = 1
    ),
    R = list(
      actual = function(df) df$R,
      proj = function(df) df$proj_R,
      flip = 1
    ),
    RBI = list(
      actual = function(df) df$RBI,
      proj = function(df) df$proj_RBI,
      flip = 1
    ),
    SBN = list(
      actual = function(df) df$SB - coalesce(df$CS, 0),
      proj = function(df) df$proj_SB - coalesce(df$proj_CS, 0),
      flip = 1
    ),
    OBP = list(
      actual = function(df) df$OBP,
      proj = function(df) df$proj_OBP,
      flip = 1
    ),
    SLG = list(
      actual = function(df) df$SLG,
      proj = function(df) df$proj_SLG,
      flip = 1
    )
  ),
  pitching = list(
    IP = list(
      actual = function(df) df$IP,
      proj = function(df) df$proj_IP,
      flip = 1
    ),
    ERA = list(
      actual = function(df) df$ERA,
      proj = function(df) df$proj_ERA,
      flip = -1
    ),
    WHIP = list(
      actual = function(df) df$WHIP,
      proj = function(df) df$proj_WHIP,
      flip = -1
    ),
    K = list(
      actual = function(df) df$SO,
      proj = function(df) df$proj_SO,
      flip = 1
    ),
    W = list(
      actual = function(df) df$W,
      proj = function(df) df$proj_W,
      flip = 1
    ),
    SVH = list(
      actual = function(df) df$SV + coalesce(df$HLD, 0),
      proj = function(df) df$proj_SV + coalesce(df$proj_HLD, 0),
      flip = 1
    )
  )
)
...
# app's expected/projected naming convention -- CRON, xCROM, xwOBA...) and
# the actual-minus-projected diff.
category_proj_cols <- function(cat) paste0(cat, c("_x", "_Diff"))

# Adds z_<cat>_x / _Diff for every core category (raw, not yet
...
# _x/_Diff columns from add_category_projection_cols(). Players with
...
add_ros_projection_score <- function(df, ros_proj_df, player_type) {
  df <- df |>
    left_join(select(ros_proj_df, -any_of("PlayerName")), by = "xMLBAMID")
...
rank_hitters <- function(df, min_pa = 50, use_impact = FALSE) {
  ...
  df <- add_cron_similarity(
    df,
    c("z_AB", "z_H", "z_R", "z_RBI", "z_SBN", "z_OBP", "z_SLG")
  )
...
rank_pitchers <- function(df, min_ip = 20, use_impact = FALSE) {
  ...
  df <- add_cron_similarity(
    df,
    c("z_IP", "z_ERA", "z_WHIP", "z_K", "z_W", "z_SVH")
  )
...
# CORE_Z_COLS, expanded to also include each category's _x/_Diff
# siblings, grouped together (actual, then its 2 projection comparisons).
core_cols_with_proj <- function(player_type) {
  unlist(lapply(CORE_Z_COLS[[player_type]], function(base) {
    c(base, paste0(base, c("_x", "_Diff")))
  }))
}

# Which of the 3 toggleable groups a core_cols_with_proj() column belongs to.
...
# of c("actual","proj","diff") toggled to "Show" in the UI).
...
build_rankings <- function(
  ...
  scale_score(
    ranked,
    c(
      grep("^z_", names(ranked), value = TRUE),
      "CROM",
      "xCROM",
      "xCROM_ROS",
      "diffCROM"
