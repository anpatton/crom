...
save_pull <- function(path, fetcher) {
  ...
  if (!ok) failures[[length(failures) + 1L]] <<- path
  invisible(ok)
}
  ...
  for (w in windows) {
    save_pull(leaders_file(pt, w), function() {
      if (w == "season") {
        ...
        fetch_fg_leaders(player_type = pt, window = "rolling", last_n = as.integer(w))
      }
    })
  }
}
...
for (pt in player_types) {
  save_pull(file.path(DATA_DIR, sprintf("proj_%s.rds", pt)),
            function() fetch_fg_projections(pt))
}

message("Pulling rest-of-season projections...")
for (pt in player_types) {
  save_pull(file.path(DATA_DIR, sprintf("rosproj_%s.rds", pt)),
            function() fetch_fg_ros_projections(pt))
}
